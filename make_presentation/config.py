...
ENDING_PRESENTATION_TEXT = {
    ...
    "mal": "Terima kasih atas perhatian anda!"
}
...
4. ТЕКУЩАЯ ДАТА: {datetime.now().date()}. Используй её как отправную точку для материала.
...
1. Проанализировать, немного сократить и разбить на 2-14 (N) слайдов в зависимости от его объема.

ВАЖНО: Если в исходном тексте уже есть какая-либо нумерация, заголовки вида "Слайд 1", "Часть 2", маркеры разделов или иная разметка на части — полностью игнорируй её. Она не должна влиять ни на количество слайдов, ни на их порядок, ни на границы между ними. Число N ты определяешь самостоятельно, исходя ТОЛЬКО из объёма и смысловой структуры текста, а не из того, как текст был размечен изначально.
...
7. ТЕКУЩАЯ ДАТА: {datetime.now().date()}. Используй её как отправную точку для материала.

Важно: это не должны быть графики или надписи.
Важно: Язык презентации должен быть - [LANGUAGE]

СТРОГИЕ ПРАВИЛА: Верни ответ строго по форме без лишних пробелов и переносов строк:
