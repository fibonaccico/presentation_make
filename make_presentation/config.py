...
- Используй ТОЛЬКО язык темы для заголовков, подзаголовков и описательных текстов
- Описания изображений ВСЕГДА пиши НА РУССКОМ языке, независимо от языка темы

Создай NUM_SLIDE (без введения/заключения) по теме \" CONTEXT \". 
Для каждого слайда:
1. Один заголовок (на языке темы)
2. 3 коротких подзаголовка (на языке темы). 
3. 3 описания на каждый из подзаголовков (очень краткий информационный текст в одно предложение с важными датами, названиями и лицами (на языке темы). 
...
Формат ответа:
Слайд {номер}
Заголовок слайда: {заголовок на языке темы}
Подзаголовок 1: {подзаголовок на языке темы}
Описание 1: {описание на языке темы}
Подзаголовок 2: {подзаголовок на языке темы}
Описание 2: {описание на языке темы}
Подзаголовок 3: {подзаголовок на языке темы}
Описание 3: {описание на языке темы}
Картинка: {описание изображения НА РУССКОМ языке}
'''

PROMPT_GENERATION_FROM_TEXT_ONE_STEP = """
Ты ИИ для генерации презентаций на основе текста.

СТРОГИЕ ПРАВИЛА ЯЗЫКА:
- Весь текстовый контент (заголовки, подзаголовки, описания) должен быть на языке исходного текста
- Описания изображений ВСЕГДА создавай ТОЛЬКО НА РУССКОМ языке, независимо от языка исходного текста

Задача:
Проанализируй предоставленный текст, сократи его и разбей на 2-10 слайдов (зависит от объема). 
Для каждого слайда создай:
1. Один заголовок (на языке исходного текста)
2. 3 коротких подзаголовка (на языке темы). 
3. 3 описания на каждый из подзаголовков (очень краткий информационный текст в одно предложение с важными датами, названиями и лицами (на языке темы). 
4. Одно описание для картинки людей, зданий, предметов, которая будет изображена на слайде (картинка) (ТОЛЬКО на русском языке). Важно: это не должны быть графики или надписи. 

Формат ответа:
Тема презентации: {тема презентации на языке исходного текста}

Слайд {номер}
Заголовок слайда: {заголовок на языке исходного текста}

Подзаголовок 1: {подзаголовок на языке исходного текста}
Описание 1: {краткий тезис в одно предложение на языке исходного текста}

Подзаголовок 2: {подзаголовок на языке исходного текста}
Описание 2: {краткий тезис в одно предложение на языке исходного текста}

Подзаголовок 3: {подзаголовок на языке исходного текста}
Описание 3: {краткий тезис в одно предложение на языке исходного текста}
