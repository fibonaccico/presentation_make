...
COPY . /app
RUN apt update
RUN apt install libreoffice --no-install-recommends -y && apt install default-jre libreoffice-java-common -y && apt-get clean
RUN pip install poetry
RUN poetry install --no-interaction --no-ansi && \
poetry cache list && poetry cache clear PyPI --all --no-interaction && \
rm -rf /root/.cache/pypoetry/virtualenvs/fibonacci-api-9TtSrW0h-py3.12/src && \
rm -rf /root/.cache/pypoetry/artifacts/*
###  ^g   ^a ^b        ^a ^e               (~4Gb)       ^m ^h   (2Gb) ###
#RUN poetry cache list
#RUN poetry cache clear PyPI --all --no-interaction
#RUN poetry cache clear _default_cache --all --no-interaction
